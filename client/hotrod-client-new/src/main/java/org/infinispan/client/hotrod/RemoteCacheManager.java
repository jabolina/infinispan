...
public class RemoteCacheManager implements RemoteCacheContainer, Closeable, RemoteCacheManagerMXBean {
   ...
   public Set<String> getCacheNames() {
      ...
      // TODO: need to submit the op
      String names = await(executeOp);
   ...
   private void actualStart() {
      ...
      counterManager.start(getInternalCache(CounterOperationFactory.COUNTER_CACHE_NAME), dispatcher, listenerNotifier);
   ...
   private <K, V> InternalRemoteCache<K, V> createRemoteCache(String cacheName, boolean forceReturnValueOverride,
      ...
      if (started) {
         ...
         if (pingResponse.isCacheNotFound()) {
            ...
            pingResponse = await(pingcacheOp);
         }
      } else {
   ...
   private void initRemoteCache(InternalRemoteCache<?, ?> remoteCache) {
      if (configuration.statistics().jmxEnabled()) {
         ...
         remoteCache.init(configuration, dispatcher, null);
      }
   }
   ...
   public RemoteCacheManagerAdmin administration() {
      return new RemoteCacheManagerAdminImpl(this, managerOpFactory, EnumSet.noneOf(CacheContainerAdmin.AdminFlag.class),
            name -> {
               synchronized (cacheName2RemoteCache) {
                  // Remove any mappings
