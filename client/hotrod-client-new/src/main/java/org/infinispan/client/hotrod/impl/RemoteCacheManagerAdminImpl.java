...
public class RemoteCacheManagerAdminImpl implements RemoteCacheManagerAdmin {
   ...
   public RemoteCacheManagerAdminImpl(RemoteCacheManager cacheManager, ManagerOperationsFactory operationsFactory, EnumSet<AdminFlag> flags, Consumer<String> remover) {
   ...
   public <K, V> RemoteCache<K, V> createCache(String name, String template) throws HotRodClientException {
      ...
      await(operationsFactory.executeOperation("@@cache@create", params));
   ...
   public <K, V> RemoteCache<K, V> getOrCreateCache(String name, String template) throws HotRodClientException {
      ...
      if (flags != null && !flags.isEmpty()) params.put(FLAGS, flags(flags));
      // TODO: actually submit
      await(operationsFactory.executeOperation("@@cache@getorcreate", params));
      return cacheManager.getCache(name);
   }
   ...
   public void removeCache(String name) {
      ...
      if (flags != null && !flags.isEmpty()) params.put(FLAGS, flags(flags));
      // TODO: actually submit
      await(operationsFactory.executeOperation("@@cache@remove", params));
   }
   ...
   public void reindexCache(String name) throws HotRodClientException {
      // TODO: actually submit
      await(operationsFactory.executeOperation("@@cache@reindex", Collections.singletonMap(CACHE_NAME, string(name))));
   }

   @Override
   public void updateIndexSchema(String name) throws HotRodClientException {
      // TODO: actually submit
      await(operationsFactory.executeOperation("@@cache@updateindexschema", Collections.singletonMap(CACHE_NAME, string(name))));
   }
   ...
   public void updateConfigurationAttribute(String name, String attribute, String value) throws HotRodClientException {
      ...
      // TODO: actually submit
      await(operationsFactory.executeOperation("@@cache@updateConfigurationAttribute", params));
   }
   ...
   public void createTemplate(String name, BasicConfiguration configuration) {
      ...
      if (flags != null && !flags.isEmpty()) params.put(FLAGS, flags(flags));
      // TODO: actually submit
      await(operationsFactory.executeOperation("@@template@create", params));
   }
   ...
   public void removeTemplate(String name) {
      ...
      if (flags != null && !flags.isEmpty()) params.put(FLAGS, flags(flags));
      // TODO: actually submit
      await(operationsFactory.executeOperation("@@template@remove", params));
   }
