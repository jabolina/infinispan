...
public class LocalPublisherManagerImpl<K, V> implements LocalPublisherManager<K, V> {
   ...
   public CompletionStage<Long> sizePublisher(IntSet segments, long flags) {
      if (!EnumUtil.containsAny(flags, FlagBitSets.CACHE_MODE_LOCAL)) {
         flags = EnumUtil.mergeBitSets(flags, FlagBitSets.CACHE_MODE_LOCAL);
      }
      SizeCommand command = commandsFactory.buildSizeCommand(segments, flags);
